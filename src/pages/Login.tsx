...
interface Props extends NavProps {
  ...
  onRequires2FA: (memberId: number) => void
}

export default function Login({ navigate, onLogin, onRequires2FA }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.requires2FA && res.memberId) {
        onRequires2FA(res.memberId)
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-[360px] bg-white border border-gray-300 rounded-sm">
        ...
        <div className="px-5 py-6">
          ...
          {error && (
            <div className="mb-4 px-3 py-2 border border-red-300 bg-red-50 rounded-sm text-sm text-red-700">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full py-2.5 bg-gray-900 text-white text-sm font-semibold rounded-sm hover:bg-black transition-colors mt-2 disabled:opacity-50">
          ...
          <p className="text-sm text-center text-gray-600">
            ...
            <button onClick={() => navigate('register-verify')} className="font-semibold text-gray-900 hover:underline">
              Register
            </button>
          </p>
        </div>
